...
export default function MypageLayout({ children }: { children: ReactNode }) {
  ...
  // `/user/projects/[id]` 또는 `/user/projects/[id]/edit`이면서 `myOpen`, `myParticipate`는 제외
  const isDetailOrEditPage =
    /^\/user\/projects\/(?!myOpen$|myParticipate$)[^/]+$/.test(pathname) ||
    /^\/user\/projects\/(?!myOpen$|myParticipate$)[^/]+\/edit$/.test(pathname);
